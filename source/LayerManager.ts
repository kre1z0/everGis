...
export class LayerManager extends ServiceGroup {
    ...
    updateService(name) {
        ...
        if (!container) error('Service is not in the group');
    ...
    replaceService(oldName, newName) {
        ...
        if (!current) error('Service is not in the group');
...
function restoreServiceParameters (container, desc, layerManager) {
    ...
    if (desc.isFolder && desc.children) {
        ...
        container.service.children.forEach(child => {
            ...
            if (childDesc) restoreServiceParameters(child, childDesc, layerManager);
        });
    }
}
